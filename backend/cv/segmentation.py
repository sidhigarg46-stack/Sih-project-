...
Isolates individual onions via robust multi-channel thresholding and watershed segmentation,
computing physical diameter (mm) and circularity index (4*pi*Area/Perimeter^2).
"""
...
def segment_onions(
    ...
    """
    Segments individual onions from the image, excluding the reference object.
    Returns a list of segmented onion metadata dictionaries.
    """
    ...
    # Background detection (works for typical light or dark surfaces)
    # Check if image corners are predominantly light or dark
    corners = [gray[0:15, 0:15], gray[0:15, -15:], gray[-15:, 0:15], gray[-15:, -15:]]
    corner_mean = np.mean([np.mean(c) for c in corners])
    
    blurred_gray = cv2.GaussianBlur(gray, (7, 7), 0)
    if corner_mean > 120:
        # Light background: foreground is darker or more saturated
        _, fg_gray = cv2.threshold(blurred_gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    else:
        # Dark background: foreground is brighter
        _, fg_gray = cv2.threshold(blurred_gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

    # Color saturation boost (onions possess vibrant red/yellow/brown or green hue)
    _, fg_sat = cv2.threshold(hsv[:, :, 1], 30, 255, cv2.THRESH_BINARY)
    # LAB 'a' channel variance for red/pink hues
    a_channel = lab[:, :, 1]
    ...
    # Exclude reference marker
    fg_combined[ref_mask > 0] = 0

    # Morphological closing to seal internal rot/skin variations
    ...
    # 3. Distance Transform + Watershed for touching onions
    ...
    for marker_id in unique_markers:
        ...
        # Shift contour coordinates to local ROI coordinate space
